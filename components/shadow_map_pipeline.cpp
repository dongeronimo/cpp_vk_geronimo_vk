...
namespace components {
    ShadowMapPipeline::ShadowMapPipeline(const std::string& name, const vk::RenderPass& rp)
        :vk::Pipeline(name, rp)
    {
        const auto device = vk::Device::gDevice->GetDevice();
        mVertexShader = vk::LoadShaderModule(device, "shadow_map.vert.spv");
        mFragmentShader = vk::LoadShaderModule(device, "shadow_map.frag.spv");
    ...
    void ShadowMapPipeline::Recreate()
    {
        const auto device = vk::Device::gDevice->GetDevice();
    ...
    void ShadowMapPipeline::Bind(VkCommandBuffer buffer, uint32_t currentFrame)
    {
        vk::Pipeline::Bind(buffer, currentFrame);

    }

    void ShadowMapPipeline::SetLightMatrix(VkCommandBuffer buffer, LightSpaceMatrixUniformBuffer& lm)
    {
        vkCmdPushConstants(buffer, mPipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(LightSpaceMatrixUniformBuffer), &lm);
    }

    void ShadowMapPipeline::Draw(components::Renderable& r, VkCommandBuffer cmdBuffer, uint32_t currentFrame)
    {
        ...
        std::array<uint32_t, 1> offsets{ modelOffset };
        ...
        vk::Pipeline::Draw(r, cmdBuffer, currentFrame);
     }

    const VkDeviceMemory ShadowMapPipeline::GetModelBufferMemory(uint32_t currentFrame) const
    {
    ...
    std::vector<VkVertexInputAttributeDescription> ShadowMapPipeline::AttributeDescription()
    {
    ...
    VkVertexInputBindingDescription ShadowMapPipeline::BindingDescription()
    {
    ...
    VkDescriptorSetLayoutBinding _MakeBinding(uint32_t binding, VkDescriptorType type, VkShaderStageFlags stageFlags) {
        VkDescriptorSetLayoutBinding b;
        b.binding = binding; // camera is at set=0, binding = 0
        b.descriptorType = type;
        b.descriptorCount = 1;
        b.stageFlags = stageFlags;
        b.pImmutableSamplers = nullptr; // Not used
        return b;
    }
    void ShadowMapPipeline::CreateDescriptorSetLayout()
    {
        const auto device = vk::Device::gDevice->GetDevice();
        VkDescriptorSetLayoutBinding modelBinding = _MakeBinding(0,
            VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, VK_SHADER_STAGE_VERTEX_BIT);
        std::array<VkDescriptorSetLayoutBinding, 1> bindings{ modelBinding  };
        VkDescriptorSetLayoutCreateInfo modelLayoutInfo{};
        modelLayoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
        modelLayoutInfo.bindingCount = bindings.size();
        modelLayoutInfo.pBindings = bindings.data();
        VkDescriptorSetLayout modelDescriptorSetLayout = VK_NULL_HANDLE;
        vkCreateDescriptorSetLayout(device, &modelLayoutInfo, nullptr, &modelDescriptorSetLayout);
    ...
    void ShadowMapPipeline::CreateDescriptorPool()
    {
        const auto device = vk::Device::gDevice->GetDevice();
    ...
    void ShadowMapPipeline::CreateModelBuffer()
    {
    ...
    void ShadowMapPipeline::CreatePipelineLayout()
    {
    ...
    void ShadowMapPipeline::CreateDescriptorSet()
    {
        const auto device = vk::Device::gDevice->GetDevice();
