...
{
    auto _device = vk::Device::gDevice;
    VkCommandBuffer commandBuffer = _device->CreateCommandBuffer("copy buffer to image");//CreateCommandBuffer(commandPool, device, "copy buffer to image");
    _device->BeginRecordingCommands(commandBuffer);
    ...
    _device->SubmitAndFinishCommands(commandBuffer);
}
...
namespace components {
        ...
        std::vector<io::ImageData*> images)
    {
        ...
        for (int i = 0; i < vkImages.size(); i++) {
            ...
            utils::CreateBuffer(images[i]->size,
                ...
                textureStagingBuffer, textureStagingMemory);
            void* textureDataPtr;
    ...
    {
        auto _device = vk::Device::gDevice;
        VkCommandBuffer commandBuffer = _device->CreateCommandBuffer("image transition command");//beginSingleTimeCommands(commandPool, device);
        _device->BeginRecordingCommands(commandBuffer);
